app.factory('loginFactory', ['$http','$q','$rootScope',function($http,$q,$rootScope){
	var address = 'http://localhost:8080/diaspora/register';
	var addressForLogin = 'http://localhost:8080/diaspora/login';
	//var firstRequestUri = 'http://localhost:8080/diaspora/'+
...
//var addressForLogin = 'http://localhost:8080/diaspora/j_spring_security_check';
...
function logout(userId){
	var deferred = $q.defer();
	$http.put('http://localhost:8080/diaspora/logout/'+userId)
	.then(function (response) {
...
function firstRequest() {
	var deferred = $q.defer();
	//$http.post('http://localhost:8080/diaspora/'+
	//'oauth/token?grant_type=password&client_id=restapp&client_secret=restapp&username='
	//+$rootScope.username+'&password='+$rootScope.password).
	$http.post()
	then(function (response) {
